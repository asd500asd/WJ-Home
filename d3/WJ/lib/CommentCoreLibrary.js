...
var CommentSpaceAllocator = (function () {
    ...
    CommentSpaceAllocator.prototype.add = function (comment) {
        if (comment.height > this._height) {
            ...
            //ltl
            var trackHeight = parseInt(this._height / 4);
            // var tracksMid = [trackHeight / 2, (trackHeight / 2 + trackHeight), (trackHeight / 2 + trackHeight * 2), (trackHeight / 2 + trackHeight * 3)];
            var tracksTop = [0, trackHeight, trackHeight*2, trackHeight*3];
            var topRange = trackHeight - comment.height;
            var trackNum = Math.floor(Math.random() * 4);
            while (trackNum == CCLlastTrackNum[0] || trackNum == CCLlastTrackNum[1]) {
                trackNum = Math.floor(Math.random() * 4);
            }
            CCLlastTrackNum.push(trackNum);
            CCLlastTrackNum.shift();
            comment.track = trackNum;
            comment.y = tracksTop[trackNum] + Math.floor(Math.random()*topRange);
...
var ScrollComment = (function (_super) {
    ...
    ScrollComment.prototype.update = function() {
        if (!this.threshold) {
            this.threshold = [];
            this.threshold[0] = this.parent.width / 2 - this.width;
            this.threshold[1] = this.threshold[0] + 69;
            /**
             * 触发状态
             * 1--未触发
             * 2--正在触发
        ...
        if (this.trigger == 1) {
            if (this.x < this.threshold[1] && this.x > this.threshold[0]) {
                this.trigger = 2;
                this.parent.dispatchEvent("showCommentDetail", this);
            }
        }
        if (this.trigger == 2) {
            if (this.x < this.threshold[0]) {
                this.trigger = 3;
                this.parent.dispatchEvent("hideCommentDetail", this);
            }
        }
