(function(window){
    ...
    // //背景颜色
    var ColorList = [['color0'], ['color1'], ['color2'], ['color3']];
    ...
    window.BeginChange = function(width,height,spacing,arrTitleHref) {
        ...
        divshift = [[-500, -200], [500, 200], [130, 130], [-30, -30], [161, 0], [-161, 0], [644, 117], [161, 0], [483, 234], [1100, 350],
                   [-232, -55], [-1100, -350], [-322, -17], [20, 20], [10, 10], [305, 111], [-222, -789], [-10, -10], [70, 50], [100, 130]];
        arrDivShift = eval(divshift);
        arrNewArea = new Array();   //新的数组排列
        arrAreaPosition = arrAreaPosition.sort(function () { return Math.random() > 0.5 ? -1 : 1; }); //数组对象随机排序
    ...
    function DrawArea(arrNewArea,width,height,spacing,arrTitleHref) {
        ...
        var color;
        var strHtml = "";
        arrTitleHref = arrTitleHref.sort(function () { return Math.random() > 0.5 ? -1 : 1; }); //数组对象随机排序
        arrColor = arrColor.sort(function () { return Math.random() > 0.5 ? -1 : 1; });         //数组对象随机排序
        arrDivShift = arrDivShift.sort(function () { return Math.random() > 0.5 ? -1 : 1; });   //数组对象随机排序
    ...
    function RightAreaHtml() {
        ...
        for (var i = 0; i < divobj.length; i++) {
            divobj.eq(i).animate({ 'left': divobj.eq(i).css('left').replace('px', '') - arrDivShift[i][0], 'top': divobj.eq(i).css('top').replace('px', '') - arrDivShift[i][1] }, ispeed);
        }
    }
}(window))
